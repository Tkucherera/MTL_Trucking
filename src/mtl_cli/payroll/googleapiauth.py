...
class GoogleApisAuthenticate(object):
    ...
    def _authenticate(self):
        ...
        if os.path.exists(os.path.join(BASE_DIR, 'config', "token.json")):
            creds = Credentials.from_authorized_user_file(os.path.join(BASE_DIR, 'config', "token.json"), self.SCOPES)
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                flow = InstalledAppFlow.from_client_secrets_file(
                    os.path.join(BASE_DIR, 'config', "credentials.json"), self.SCOPES
                )
                creds = flow.run_local_server(port=0)
            # Save the credentials for the next run
            with open(os.path.join(BASE_DIR, 'config', "token.json"), "w") as token:
