...
def pay_calc(drivers: list, trip_sheet_name: str, pay_date=None):
    spreadsheet_id = None
    spreadsheet_info = open(os.path.join(BASE_DIR, 'config', 'spreadsheetId'), 'r')
    spreadsheet_id = spreadsheet_info.readline().strip()

    if not spreadsheet_id:
        logger.error("SpreadSheetId Needed (add to config directory)")
        raise Exception("SpreadSheetId Needed (add to config directory)")
    ...
    for driver in drivers:
        ...
        trucker = DriverCalculationSheet(driver, d.stringify_date(d.end_week_date, "%m/%d/%y"), calculation_sheet_values)
        ...
        if len(rows) == 0:
            ...
            logger.info(f"Could Not find any loads for {trucker.name} from {dates[0]} to {dates[-1]}")
            continue
        for row in rows:
            if len(row) >= 32:
                try:
                    if row[31].strip() == 'Delivered':
                        trucker.add_trip(Trip(date=row[0], driver=row[1], broker=row[3], rate_con=row[4], rate=row[20], truck_name=row[2]))
                    else:
                        logger.info(f'Cancelled Trip: {row[0]}  {row[1]}   {row[3]}   {row[4]}   {row[20]}')
                except Exception as e:
                    logger.error(f'Adding trip for driver {trucker.name} failed with error {e}')
