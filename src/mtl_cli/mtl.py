...
def main():
    ...
    if args.date:
        pay_date = args.date
        try:
            t = DeterminePayPeriod(pay_date)
            date = t.stringify_date(t.end_week_date, "%m/%d/%y")
        ...
        try:
            ...
            for driver in drivers_argument:
                ...
                try:
                    ...
                    #logger.error("Percentage has to be in integer or decimal format (Numeric)")
                    sys.exit(1)
    ...
    if args.json:
        ...
        if os.path.exists(file):
            ...
            #logger.error(f'Could not find file try inputting full path:  ${file}')
            sys.exit(1)
